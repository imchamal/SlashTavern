...
function openSearchInputPanel() {
    const panel = createPanel('ct-change-panel', '검색');
    const body = getPanelBody(panel);
...
export function registerFindChangeCommands() {
    SlashCommandParser.addCommandObject(SlashCommand.fromProps({
        name: 'fr',
        helpString: '채팅에서 검색하고, 필요하면 바로 바꿀 수도 있습니다. 사용법: /fr 키워드 (검색만), /fr 원본텍스트/바꿀텍스트 (검색+바꾸기), 인자 없이 /fr 만 입력하면 옵션과 범위를 고를 수 있는 입력 패널이 뜹니다.',
        unnamedArgumentList: [
        ...
        callback: async (_a, value) => {
            ...
            const find = slashIdx === -1 ? raw.trim() : raw.slice(0, slashIdx);
            const replace = slashIdx === -1 ? '' : raw.slice(slashIdx + 1);
